...
function rbs_updatepackages()
{
	clear
	log -infos "Entering in Packages Update function ..."
	__file="$ROOTDIR/bin/libraries.txt"
	while IFS=, read __state __package || [[ -n $line ]]; do
		if [ "$__state" == "0" ]; then
			log -infos "__state\t\t= $__state"
			log -infos "__package\t\t= $__package"
			check -installpackage $__package
			if [ $? == 0 ]; then
				log -infos "error\t\t= $?"
				log -infos "Replace line: '$__state,$__package' '1,$__package' '$__file'"
				text -lineReplaceWith "$__state,$__package" "1,$__package" "$__file"
				log -infos "error\t\t= $?"
			fi
		elif [ "$__state" == "1" ]; then
			log -warning "__state\t\t= $__state"
			log -warning "The package '$__package' exists already !"
		fi
	done < $__file
}
